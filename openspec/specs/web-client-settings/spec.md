...
Users tab calls. Tag management lives on its own `/tags` page — see
`web-client-tags` — not as a Settings tab.
...
- **THEN** only the Profile tab is shown in the tab list
...
- **THEN** both the Profile and Users tabs are shown
...
visitor (no `is_admin` requirement), positioned between Profile and the
...
- **THEN** the tab list includes "My Invitations" alongside Profile
