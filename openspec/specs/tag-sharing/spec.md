# tag-sharing Specification

## Purpose

Sharing a tag with other registered users: the two permission tiers
(`view`, `append`) and what each grants, the `tag_shares` model, the
share-management endpoints (`/api/tags/{id}/shares`), the email-invite
flow, and revocation/self-leave semantics. Unlike `account-sharing`, there
is no shareable "owner" tier — only a tag's real owner may ever edit its
metadata, lifecycle, or shares. See `entry-tags` for how the `append` tier
plugs into an entry's `tag_id` validation, and `category-sharing` for the
parallel model this one deliberately mirrors.
