# web-client-tags Specification

## Purpose

The authenticated `/tags` page: the sidebar link, the auth gate, the flat
list of the caller's own tags, and the only surface (other than the entry
form's inline create-while-tagging flow) where a tag can be created or
renamed — including create/rename, disabling/enabling, and deleting —
plus the flat "Shared with me" section and the Share entry point into
`web-client-tag-sharing`. See `entry-tags`/`tag-sharing` for the backend
capabilities and `web-client-entries` for how a tag is picked on an entry.
