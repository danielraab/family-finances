...
visitors to the public placeholder, and a card per account showing its
title, financial institute, and live balance, with a shortcut to add a
new entry against it. See `web-client-shell` for the sidebar's "Home"
link and `web-client-accounts` for the equivalent `/accounts` overview.
