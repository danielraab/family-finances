...
- [x] 1.1 `entries.index.tsx`: add `selectedIds: Set<string>` state; a
...
- [x] 1.2 Add a `BulkActionToolbar` component, rendered above the table
...
- [x] 2.1 Add `frontend/src/lib/bulkRun.ts` (or similar): given a list of
...
- [x] 2.2 Add `BulkActionRunModal` (or similar): opens with a task list,
  ...
  and **Close** buttons.
- [x] 2.3 Add a small per-entry failure-reason mapper (HTTP status/error
...
- [x] 3.1 Add a small `Dialog` (opened from the toolbar) with the same
...
- [x] 3.2 On Apply: build one task per selected entry —
...
- [x] 4.1 Add tags: a `Dialog` with `TagInput` (existing-tags filter:
...
- [x] 4.2 Remove tags: a `Dialog` with `TagInput`, but its `existingTags`
...
- [x] 4.3 Set tags: a `Dialog` with `TagInput` (same existing-tags filter
...
- [x] 5.1 When enabled (single shared `account_id` across the selection),
...
- [x] 8.1 Frontend: `pnpm lint`, `pnpm exec tsc`, `pnpm build` from
  `frontend/`.
- [x] 8.2 Manually exercise the golden path in a browser: select a mix of
