...
real owner as the target user SHALL be rejected (`422`) — the real owner
...
- **THEN** the request is rejected (`422`) and no share is changed
...
- **THEN** the request is rejected (`422`) and no share is created
...
- **THEN** the request is rejected (`422`) — there is no share row to remove
