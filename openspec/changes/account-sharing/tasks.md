...
- [x] 2.1 Add `AccountShare` type + `Permission` enum (`view`, `append`,
  `entry_admin`, `owner`) with a rank/comparison helper (`AtLeast(other)`)
  to `account.go`.
- [x] 2.2 `Store`: `CreateOrUpdateShare(ctx, accountID, userID, permission,
...
- [x] 2.3 `Access(ctx, accountID, callerID) (Access, error)` replaces
...
- [x] 2.4 `List`/`VisibleIDs` widen to accounts the caller owns **or** has
  any share on (`UNION` in Postgres; a set union in the memory store).
- [x] 2.5 Implement all of the above in `internal/storage/memory` and
  `internal/storage/postgres`.

## 3. Backend: `internal/account` — sharing service + authorization

- [x] 3.1 New `UserLookup` interface (`ByEmail(ctx, email) (userID,
...
- [x] 3.2 `Service.ListShares(ctx, callerID, accountID) ([]AccountShare,
...
- [x] 3.3 `Service.InviteShare(ctx, callerID, accountID, email, permission)
...
- [x] 3.4 `Service.UpdateSharePermission`/`RevokeShare(ctx, callerID,
...
- [x] 3.5 New sentinel errors (`ErrForbidden` if one doesn't already exist
  in this package, or reuse an existing shape) mapped in
  `httpapi/respond.go` (`403`).
- [x] 3.6 `handler.go`: `GET`/`POST /api/accounts/{id}/shares`,
...
- [x] 3.7 Unit + handler tests: invite matched/unmatched (with
...
- [x] 4.1 `Service.Get`/`Update`/`Disable`/`Enable`/`Delete`: replace the
  `ownerID == caller` check with `Access(...).Permission ==
  PermissionOwner` (or `!= ""` for `Get`, matching the "view+ can read"
  rule).
- [x] 4.2 `Service.Update`: reject a request whose body includes `type_id`
...
- [x] 4.3 `handler.go`: `Account` response gains `permission` (the caller's
  own effective tier) and, when the caller isn't the real owner,
  `owner_name`/`shared: true`.
- [x] 4.4 Tests: view/append/entry_admin tiers get `403` from
...
- [x] 5.1 Rename `OwnerID` → `CreatedBy` throughout `entry.go`,
  `service.go`, `store.go`, both storage backends' SQL and Go structs.
- [x] 5.2 Replace `AccountLookup.Owner` with `Access(ctx, accountID,
...
- [x] 5.3 `checkAccount`: require `Access.Permission.AtLeast(PermissionAppend)`
  instead of `accOwner == callerID`.
- [x] 5.4 `Store.Get(ctx, id)`: drop the `ownerID` parameter — plain lookup
  by id (excluding soft-deleted), no caller-awareness.
- [x] 5.5 `Store.List`/`Sum`/`FlowSummary`/`Balance`: drop the `ownerID`
  ...
  reaching the store.
- [x] 5.6 `Store.Update`/`SoftDelete`: drop `ownerID` — the service has
  already authorized by the time either is called.
- [x] 5.7 `Service.Get(ctx, callerID, id)`: `store.Get(id)` →
  `accounts.Access(entry.AccountID, callerID).Permission.AtLeast(View)` or
  `ErrNotFound`.
- [x] 5.8 `Service.Update`/`Delete(ctx, callerID, id, ...)`: `store.Get(id)`
...
- [x] 5.9 `Service.Create`: `checkAccount` per 5.3; created row's
  `CreatedBy` is always `callerID`.
- [x] 5.10 Implement all store signature changes in `internal/storage/
  memory` and `internal/storage/postgres`.
- [x] 5.11 Tests: append edits own / cannot edit another's (`403`);
...
- [x] 6.1 `internal/auth`: add `ByEmail` and `InvitingEnabled` to
  `Service` (thin wrappers over existing lookups/config), satisfying
  `account.UserLookup` structurally.
- [x] 6.2 `internal/mailer` (or a new template alongside the existing
  invite/magic-link ones): a share-notification email — account title,
  granter's name, permission, and an application link.
- [x] 6.3 `main.go`: wire `account.WithUserLookup(authSvc)` and the mailer
  into `account.NewService(...)`.

## 7. API contract

- [x] 7.1 `openapi/openapi.yaml`: `AccountShare` schema (`user_id`, `name`,
...
- [x] 7.2 `cd backend && go generate ./...` to sync `backend/openapi.yaml`.
- [x] 7.3 `cd frontend && pnpm generate:api` to regenerate
  `src/api/schema.d.ts`.

## 8. Frontend: shared badge, owner name, Share button

- [x] 8.1 `AccountLabel.tsx`: accept the account's `shared`/`owner_name`
  fields, rendering a shared icon + owner name alongside the existing
  icon/colour badge and title when `shared` is true.
- [x] 8.2 `accounts.index.tsx`, `accounts.$accountId.index.tsx`,
...
- [x] 8.3 `useAccountsWithBalances.ts` (or wherever `GET /api/accounts` is
...
- [x] 9.1 `accounts.$accountId.index.tsx`: hide the edit link,
  disable/enable, and delete actions unless `account.permission ===
  "owner"`.
- [x] 9.2 `accounts.$accountId.edit.tsx`: redirect to the detail page if
...
- [x] 10.1 New route `frontend/src/routes/accounts.$accountId.sharing.tsx`
  — auth-gated, fetches `GET /api/accounts/{id}/shares` on mount.
- [x] 10.2 Render the real owner as a fixed first row, then every share
  (name, permission, granted-by), with a Leave action only on the
  visitor's own row.
- [x] 10.3 When `account.permission === "owner"`: render the invite form
  ...
  confirmed).
- [x] 10.4 Invite form submit handling: on `matched: true`, insert the new
...
- [x] 10.5 Leave and Revoke both go through the existing
  `@headlessui/react` `Dialog` confirmation pattern.

## 11. Frontend: entries — creator annotation + permission gating

- [x] 11.1 `entries.index.tsx`, the account detail page's recent-entries
  list: render `created_by_name` as a small annotation whenever
  `created_by !== currentUser.id`.
- [x] 11.2 `entries.$entryId.edit.tsx`: render the entry read-only (no
...
- [x] 11.3 `entries.new.tsx`: the account `<select>` (when not preset via
  `?account_id=`) includes every account with `append`+ permission,
  excluding `view`-only ones.
- [x] 11.4 `reports.tsx`: the account filter includes every account with
  any permission (view and up).

## 12. i18n

- [x] 12.1 Add new keys to `frontend/src/i18n/locales/en.json` first, then
