...
- [x] 1.1 Extract the accounts+types+balances fetch effects currently inline
...
- [x] 1.2 Update `accounts.index.tsx` to call the new hook instead of its
      inline effects; verify `/accounts` still lists accounts and balances
      identically (manual check in the browser).
- [x] 1.3 Run `pnpm exec tsc` and `pnpm lint` from `frontend/`; verify clean.

## 2. `/home` route

- [x] 2.1 Create `frontend/src/routes/home.tsx` as a single self-contained
...
- [x] 2.2 In the authenticated branch, call `useAccountsWithBalances()` and
      render one card per account (see task 3); when `accounts` is an empty
      array, render the empty state (task 4) instead.
- [x] 2.3 Verify manually: an anonymous visitor navigating directly to
      `/home` is redirected to `/`.

## 3. Account card component

- [x] 3.1 Create `frontend/src/components/AccountCard.tsx` (or colocate in
      ...
      `accounts.index.tsx`).
- [x] 3.2 Wrap the card body in a `Link` to `/accounts/{id}` (mirroring
      ...
      `accounts.index.tsx`.
- [x] 3.3 Lay the cards out in a responsive grid (e.g. Tailwind `grid
...
- [x] 3.4 Verify manually: balances render in red/green/neutral matching
      `/accounts`'s coloring for the same accounts.

## 4. Empty state

- [x] 4.1 Add a home-specific empty state (shown when the authenticated
      ...
      empty state.
- [x] 4.2 Verify manually with a fresh account that has no accounts yet.

## 5. Sidebar navigation

- [x] 5.1 In `frontend/src/components/Sidebar.tsx`, change the `NAV` array's
...
- [x] 5.2 Verify manually: clicking "Home" in the sidebar while
...
- [x] 6.1 Add new `en.json` keys under a `home` (or similarly scoped)
...
- [x] 7.2 Manually exercise the full flow (`docker compose up -d db`, `go
