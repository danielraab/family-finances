...
default currency, displayed decimal places, and week start — in a
`user_settings` table with one row per user keyed by `user_id`, each
preference column nullable. A missing row and a row whose column is `NULL`
SHALL be treated identically: both resolve to a hardcoded application
default (`language: "en"`, `timezone: "UTC"`, `default_currency: "EUR"`,
`displayed_decimal_places: 2`, `week_start: "monday"`). No row SHALL be
created automatically at account creation; a row SHALL be created (or
updated) only when a preference is explicitly set.
...
- **WHEN** a user's `user_settings` row has `language = "de"` and
  `timezone`, `default_currency`, `displayed_decimal_places`, and
  `week_start` all `NULL`
...
accepted only as `monday` or `sunday`. An invalid value for any field SHALL
be rejected without changing any other field's stored value.
...
authenticated user's resolved settings (all five fields always populated).
...
`displayed_decimal_places`, `week_start`; only the fields present SHALL be
changed, and the response SHALL be the resulting resolved settings.
