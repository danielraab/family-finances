...
### Requirement: Observable HTTP behavior is unchanged by the restructure

The restructure SHALL NOT change any client-observable behavior. `GET
/api/healthz` SHALL still return `200` with body `ok`; static file serving and
the embedded `404.html` not-found page SHALL be byte-for-byte identical; and
`go build`, `go test ./...`, and `go run .` SHALL still succeed from a fresh
clone with no frontend build present.

#### Scenario: Healthcheck probe still works

- **WHEN** `server healthcheck` runs against a running server
- **THEN** it exits `0` on a healthy server and non-zero otherwise, as before

#### Scenario: Fresh-clone build and test

- **WHEN** a developer runs `go build .` and `go test ./...` in `backend/`
  immediately after cloning, without building the frontend
- **THEN** both succeed
