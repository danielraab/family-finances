...
- [x] 1.1 Add `AccountID *string` to `entry.Update`
      (`backend/internal/entry/entry.go`), documenting that a non-nil value
      changes the entry's account.
- [x] 1.2 Extract the account ownership/disabled check shared by
...
- [x] 1.3 In `Service.Update`, when `upd.AccountID != nil`, call the same
...
- [x] 1.4 Verify a unit test moving an entry to an account owned by a
      different user is rejected with `ErrInvalidValue` and the entry is
      unchanged.
- [x] 1.5 Verify a unit test moving an entry to a disabled account owned by
      the caller is rejected with `ErrAccountDisabled` and the entry is
      unchanged.
- [x] 1.6 Verify a unit test moving a `kind: balance_adjustment` entry
      succeeds the same way as a `transaction` entry.
- [x] 1.7 Verify a unit test moving an entry between accounts with
      different `currency` values succeeds and leaves `amount` unchanged.
- [x] 1.8 Update `storage/memory` and `storage/postgres` `Store.Update`
...
- [x] 2.1 Add `account_id` to `entryUpdateBody`
      (`backend/internal/entry/handler.go`) and pass it through to
      `Update.AccountID`.
- [x] 2.2 Update `openapi/openapi.yaml`'s `EntryUpdate` schema to include
      ...
      `400`.
- [x] 2.3 Run `cd backend && go generate ./...` to sync
      `backend/openapi.yaml` from the root spec; verify no diff remains
      uncommitted.
- [x] 2.4 Add/extend a handler test in `backend/internal/entry/handler_test.go`
...
- [x] 2.5 Verify `gofmt -l .`, `go vet ./...`, and `go test ./...` are clean
      from `backend/`.

## 3. Frontend: generated types

- [x] 3.1 Run `cd frontend && pnpm generate:api` to regenerate
...
- [x] 4.1 In `entries.$entryId.edit.tsx`, replace the disabled account
...
- [x] 4.2 On unlock, fetch/reuse the visitor's own accounts (mirroring how
...
- [x] 4.3 Ensure the entry's current account still appears in the unlocked
...
- [x] 4.4 Add the cancel button next to the unlocked field: clicking it
...
- [x] 5.1 Compare the currently selected account's `currency` to the
...
- [x] 5.2 On submit, if the selected `account_id` still equals the entry's
      original value, submit immediately as today (no behavior change);
      verify the existing save flow is unaffected.
- [x] 5.3 On submit, if the selected `account_id` differs, open a
      ...
      immediately.
- [x] 5.4 Wire the dialog's confirm action to submit the `PATCH` (including
...
- [x] 6.1 Add new `en.json` keys for: the unlock button's accessible label,
...
- [x] 7.1 Run `pnpm lint`, `pnpm exec tsc`, and `pnpm build` from
      `frontend/`; verify all pass.
- [x] 7.2 Manually exercise the full flow in a running app (`docker compose
...
- [x] 7.3 Manually verify the disabled-target rejection surfaces as an
