...
- [x] 1.1 Migration `0006_account_types.sql`: `account_types(id uuid PK,
  name text NOT NULL UNIQUE, created_at timestamptz NOT NULL DEFAULT now())`.
- [x] 1.2 Migration `0007_accounts.sql`: `accounts(id uuid PK, owner_id uuid
  ...
  index on `(owner_id)`.
- [x] 1.3 Migration `0011_account_disabled.sql`: `ALTER TABLE accounts ADD
  COLUMN disabled boolean NOT NULL DEFAULT false`.
- [x] 1.4 Create `backend/internal/account/` in the four-file shape:
...
- [x] 1.5 Implement `Store` in `internal/storage/memory` and
...
- [x] 1.6 Wire into `main.go`, mount at `/api/accounts` and
  `/api/account-types`.
- [x] 1.7 Unit tests (service, memory store) + handler tests, including:
...
- [x] 2.1 Migration `0008_categories.sql`: `categories(id uuid PK, parent_id
  uuid REFERENCES categories(id), name text NOT NULL, created_at timestamptz
  NOT NULL DEFAULT now())`, unique `(parent_id, name)`.
- [x] 2.2 Create `backend/internal/category/` in the four-file shape:
...
- [x] 2.3 Implement `Store` in `internal/storage/memory` and
  `internal/storage/postgres`; delete rejected (`409`) when the category has
  children or is referenced by a non-deleted entry.
- [x] 2.4 Wire into `main.go`, mount at `/api/categories`.
- [x] 2.5 Unit tests + handler tests: non-admin blocked from writes;
...
- [x] 3.1 Migration `0009_tags.sql`: `tags(id uuid PK, owner_id uuid NOT
  NULL REFERENCES users(id), name text NOT NULL, created_at timestamptz NOT
  NULL DEFAULT now())`, unique `(owner_id, name)`.
- [x] 3.2 Create `backend/internal/tag/` in the four-file shape, including
  the `TagLookup` interface `internal/entry` will consume; CRUD restricted
  to `owner_id = caller`.
- [x] 3.3 Implement `Store` in `internal/storage/memory` and
  `internal/storage/postgres`.
- [x] 3.4 Wire into `main.go`, mount at `/api/tags`.
- [x] 3.5 Unit tests + handler tests: a user cannot see or reference
  another user's tags; duplicate name for the same owner rejected.

## 4. Backend: `internal/entry`

- [x] 4.1 Migration `0010_entries.sql`: `entries(id bigserial PK, owner_id
...
- [x] 4.2 Create `backend/internal/entry/` in the four-file shape:
...
- [x] 4.3 `Create` rejects (`422`, `ErrAccountDisabled`) when the target
  account's `disabled` is true.
- [x] 4.4 Implement `Store` in `internal/storage/memory` and
...
- [x] 4.5 Wire into `main.go`: construct `account.Service`,
  `category.Service`, `tag.Service` first, pass them as
  `entry.NewService`'s `AccountLookup` / `CategoryLookup` / `TagLookup`.
- [x] 4.6 Unit tests + handler tests covering: transaction without a
...
- [x] 6.2 `cd backend && go generate ./...` (sync `backend/openapi.yaml`).
- [x] 6.3 `cd frontend && pnpm generate:api` (regenerate
  `src/api/schema.d.ts`).
- [x] 6.4 Add `internal/openapicheck.AssertResponse` assertions to the new
