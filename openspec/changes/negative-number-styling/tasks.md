...
- [ ] 1.1 `frontend/src/lib/amount.ts`: add `amountColorClass(amount:
...
- [ ] 3.1 New `frontend/src/components/SignedAmountInput.tsx`: props for
...
- [ ] 3.2 Toggle button has a translated `aria-label` reflecting its
  action (new i18n key).

## 4. Wire into the entry form

- [ ] 4.1 `frontend/src/routes/entries.new.tsx`: for `kind === "transaction"`,
...
- [ ] 4.2 `frontend/src/routes/entries.$entryId.edit.tsx`: same branching
...
- [ ] 5.1 Add new keys to `frontend/src/i18n/locales/en.json` first, then
...
- [ ] 6.1 `cd frontend && pnpm lint && pnpm exec tsc && pnpm build`.
- [ ] 6.2 Manual pass in the browser: confirm red/black/green on negative/
