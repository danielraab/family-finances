...
- [ ] 2.1 `tag.go`: add `Disabled bool` and `EntryCount int` to `Tag`
      (`json:"entry_count"`).
- [ ] 2.2 `store.go`: add `SetDisabled(ctx, ownerID, id string, disabled
      ...
      unchanged.
- [ ] 2.3 Implement `SetDisabled` and `Usable` in `internal/storage/memory`
...
- [ ] 4.3 Update the hand-rolled `TagLookup` fakes (`newStubTags()` in
      `internal/entry/handler_test.go` and
      `internal/entry/service_scenarios_test.go`) to implement `Usable`.
- [ ] 4.4 Service tests: creating an entry with a disabled `tag_ids` entry
...
- [ ] 5.1 `openapi/openapi.yaml`: `Tag` — add `disabled` (boolean,
...
- [ ] 7.1 `frontend/src/routes/settings.tags.tsx` (new route,
      ...
      form.
- [ ] 7.2 Add "Tags" to the tab nav in `frontend/src/routes/settings.tsx`,
      alongside Common, My Invitations, and Account Types (open to every
      authenticated visitor).
- [ ] 7.3 Create/rename go through `POST`/`PATCH /api/tags`. Disable and
...
- [ ] 8.1 `frontend/src/routes/entries.new.tsx` and `entries.$entryId.
...
- [ ] 10.1 `cd backend && gofmt -l . && go vet ./... && go test ./...`
      ...
      real Postgres, per `backend/AGENTS.md`).
...
- [ ] 10.3 Manual pass: create/rename/disable/enable/delete a tag on the
      ...
      delete does.
- [ ] 10.4 Update `backend/AGENTS.md` (new "Tags" section, mirroring
...
- [ ] 11.1 Apply this change's `specs/entry-tags`, `specs/account-entries`,
