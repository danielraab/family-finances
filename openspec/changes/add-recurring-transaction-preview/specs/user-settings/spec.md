...
default currency, displayed decimal places, week start, and the recurring
preview horizon — in a `user_settings` table with one row per user keyed
by `user_id`, each preference column nullable. A missing row and a row
whose column is `NULL` SHALL be treated identically: both resolve to a
hardcoded application default (`language: "en"`, `timezone: "UTC"`,
`default_currency: "EUR"`, `displayed_decimal_places: 2`,
`week_start: "monday"`, `recurring_preview_horizon: "end_of_this_month"`).
No row SHALL be created automatically at account creation; a row SHALL be
created (or updated) only when a preference is explicitly set.
...
- **WHEN** a user's `user_settings` row has `language = "de"` and every
  other column, including `recurring_preview_horizon`, `NULL`
...
digits than are ever stored would be meaningless. `week_start` SHALL be
accepted only as `monday` or `sunday`. `recurring_preview_horizon` SHALL be
accepted only as one of `1_month`, `2_months`, `3_months`,
`end_of_this_month`, `end_of_next_month`, `end_of_this_year`. An invalid
value for any field SHALL be rejected without changing any other field's
stored value.
...
- **WHEN** a settings update is submitted with `week_start: "tuesday"`
- **THEN** the request is rejected and no field is changed

#### Scenario: Invalid recurring preview horizon rejected

- **WHEN** a settings update is submitted with
  `recurring_preview_horizon: "6_months"`
- **THEN** the request is rejected and no field is changed

### Requirement: Settings endpoints

`GET /api/settings` SHALL require authentication and SHALL return the
authenticated user's resolved settings (all six fields always populated).
...
`displayed_decimal_places`, `week_start`, `recurring_preview_horizon`; only
the fields present SHALL be changed, and the response SHALL be the
...
- **WHEN** an authenticated user with no stored preferences calls
  `GET /api/settings`
- **THEN** the response is `200` with the hardcoded defaults, including
  `recurring_preview_horizon: "end_of_this_month"`
...
#### Scenario: Updating week start

- **WHEN** an authenticated user calls `PUT /api/settings` with
  `{ "week_start": "sunday" }`
- **THEN** the response shows `week_start: "sunday"`, and every other field
  is unchanged

#### Scenario: Updating the recurring preview horizon

- **WHEN** an authenticated user calls `PUT /api/settings` with
  `{ "recurring_preview_horizon": "3_months" }`
- **THEN** the response shows `recurring_preview_horizon: "3_months"`, and
