...
- [x] 1.1 In `openapi/openapi.yaml`, add `GET /api/recurring-transactions
...
- [x] 1.2 Add `RecurringTransactionPreviewItem` schema (`recurring_transaction_id`,
      `account_id`, `account_currency`, `title`, `description`,
      `category_id`, `counterparty`, `location`, `tag_ids`, `amount`,
      `booking_timestamp`, `overdue`).
- [x] 1.3 Add `recurring_preview_horizon` (enum: `1_month`, `2_months`,
...
- [x] 1.4 Add `show_recurring_preview` (boolean) to `DashboardCardConfig`,
      per `dashboard-cards`'s modified requirement.
- [x] 1.5 Sync `backend/openapi.yaml` (`cd backend && go generate ./...`)
...
- [x] 2.1 In `backend/internal/recurringtransaction/recurringtransaction.go`,
...
- [x] 2.2 Add a `PreviewFilter` type (`AccountIDs`, `CategoryID`,
...
- [x] 2.3 Add `backend/internal/recurringtransaction/handler.go`'s
      `GET /preview` route: parses query params, calls `Service.Preview`,
      maps `ErrInvalidValue` (missing `to`) to `400`.
- [x] 2.4 Unit tests: basic multi-occurrence projection; the single
...
- [x] 3.1 Add migration
      ...
      `user_settings`.
- [x] 3.2 In `backend/internal/settings`, add the field to the resolved
      settings struct, its hardcoded default (`end_of_this_month`), and
      validation against the six allowed enum values.
- [x] 3.3 Unit tests: default resolution (missing row / `NULL` column),
...
- [x] 4.1 Add a shared helper (e.g. `lib/recurringPreview.ts`) resolving a
...
- [x] 4.2 Add the horizon-preset resolution functions (1/2/3 months from
...
- [x] 5.1 `entries.new.tsx`: accept an optional `booking_timestamp` search
...
- [x] 6.1 Build a shared `UpcomingBlock` component (row: title,
...
- [x] 6.2 Add the new `dashboard.*`/`entries.*`/`reports.*` i18n keys this
      component needs (heading, empty state, overdue hint) to `en.json`
      first, then `de.json`.

## 7. Frontend: /entries wiring

- [x] 7.1 `entries.index.tsx`: add the `show_recurring` boolean URL search
      param (default `false`) and its filter-row toggle control.
- [x] 7.2 When on, fetch the preview using the ledger's current
...
- [x] 8.1 `reports.tsx`: add the `show_recurring` boolean URL search param
      and its toggle control alongside the existing filter controls —
      changing it updates the URL only, per the page's existing
      no-auto-fetch rule.
- [x] 8.2 "Generate report" additionally fetches the preview (using the
...
- [x] 9.1 `CardFormDialog.tsx`: add the `show_recurring_preview` toggle,
      shown only for `entry_list`/`bar_chart` card types, included in the
      submitted config.
- [x] 9.2 `EntryListCard.tsx`: when `config.show_recurring_preview` is
...
- [x] 10.1 Extend `BarChart`/`BarChartDatum` with an optional stacked
...
- [ ] 10.2 Add muted `INCOME_FILL_PROJECTED`/`OUTCOME_FILL_PROJECTED`
      ...
      (Opacity-reduced variants of the existing hues are wired in as a
      placeholder — `INCOME_PROJECTED_FILL`/`OUTCOME_PROJECTED_FILL` in
      `BarChartCard.tsx`, flagged with a TODO — but not yet run through
      the actual validator, which wasn't available in this environment.)
- [x] 10.3 `BarChartCard.tsx`: when `config.show_recurring_preview` is
...
- [x] 11.1 `settings.index.tsx`'s Profile tab: add the recurring preview
...
- [x] 12.1 Add every remaining new user-facing string (the `show_recurring`
...
- [x] 13.1 Backend: `gofmt -l .`, `go vet ./...`, `go test ./...` (from
      `backend/`), including the postgres integration suite against the
      local compose DB.
- [x] 13.2 Frontend: `pnpm lint`, `pnpm exec tsc`, `pnpm build` (from
      `frontend/`).
...
- [x] 13.4 Confirm no drift: `openapi/openapi.yaml` and
