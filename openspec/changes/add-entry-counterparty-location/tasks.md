...
- [x] 1.1 Add migration `0024_entry_counterparty_location.sql` adding
...
- [x] 2.1 `entry.go`: add `Counterparty *string` and `Location *string` to
  `Entry`, `New`, and `Update` (plain nullable strings, mirroring
  `Description`'s shape — no `OptionalID` needed, see design.md).
- [x] 2.2 `entry.go`'s `validateNew` (and the equivalent update-side
...
- [x] 2.3 `store.go`: add `ListInUseCounterparties(ctx, ownerID) ([]string,
  error)` to the `Store` interface, mirroring `account.Store`'s
  `ListInUseTypes`.
- [x] 2.4 `service.go`: add `ListInUseCounterparties` passthrough method,
  mirroring `account.Service.ListInUseTypes`.
- [x] 2.5 `handler.go`: add `GET /api/entries/counterparties` (register
...
- [x] 3.1 `postgres/entry.go`: add `counterparty`/`location` to
  ...
  `ListInUseTypes`).
- [x] 3.2 `postgres/entry.go`: extend the `Query` free-text search
  (currently `title ILIKE %q% OR description ILIKE %q%`, around
  `entry.go:320-322`) to also match `counterparty`.
- [x] 3.3 `storage/memory`: mirror the same field storage, search
  extension, and `ListInUseCounterparties` for the in-memory test store.

## 4. API contract

- [x] 4.1 Update `openapi/openapi.yaml`: `Entry`, `EntryCreate`,
...
- [x] 4.2 Run `cd backend && go generate ./...` to sync
  `backend/openapi.yaml`.
- [x] 4.3 Run `cd frontend && pnpm generate:api` to regenerate
  `frontend/src/api/schema.d.ts`.
- [x] 4.4 Add response-conformance assertions
  (`internal/openapicheck.AssertResponse`) to the new/changed handler
  tests, per `backend/AGENTS.md`.

## 5. Backend tests

- [x] 5.1 `entry_test.go`/`service_scenarios_test.go`: creating a
...
- [x] 5.2 `service_test.go` (or equivalent): `ListInUseCounterparties` is
...
- [x] 5.3 `handler_test.go`: `GET /api/entries/counterparties` requires
...
- [x] 5.4 `postgres/entry_test.go`: integration coverage for the new
...
- [x] 6.1 Add Leaflet (and its CSS) as a `frontend/` dependency via
  `pnpm add`; import its stylesheet once, near the other global style
  imports.
- [x] 6.2 Create `frontend/src/components/LocationField.tsx`: a text input
...
- [x] 6.3 Create `frontend/src/components/LocationPickerModal.tsx`: an
...
- [x] 6.4 Create `frontend/src/components/LocationPreviewModal.tsx`: a
  small, read-only Leaflet/OSM map centered on a given `{lat,lng}`, with a
  static (non-draggable) marker and no editing controls.
- [x] 6.5 Add a small `frontend/src/lib/location.ts` helper: `parseLocation
...
- [x] 7.1 `entries.new.tsx`/`entries.$entryId.edit.tsx`: add a
...
- [x] 9.1 Add `entries.form.counterparty*`, `entries.form.location*`
...
- [x] 10.1 Backend: `gofmt -l .`, `go vet ./...`, `go test ./...` from
  `backend/`.
- [x] 10.2 Frontend: `pnpm lint`, `pnpm exec tsc`, `pnpm build` from
  `frontend/`.
- [x] 10.3 Manually exercise the golden path in a browser: create a
