...
type CategoryPermission = components["schemas"]["CategoryPermission"];

const PERMISSIONS: Exclude<CategoryPermission, "owner">[] = ["view", "append"];
...
function SharingPage() {
  ...
  const [shares, setShares] = useState<CategoryShare[] | null>(null);

  const [inviteEmail, setInviteEmail] = useState("");
  const [invitePermission, setInvitePermission] =
    useState<CategoryPermission>("view");
  ...
  const load = useCallback(() => {
    let cancelled = false;
    api
      .GET("/api/categories/{id}", { params: { path: { id: categoryId } } })
      .then(({ data }) => {
        if (!cancelled) setCategory(data ?? null);
      });
    api
  ...
  async function handleInvite(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, response } = await api.POST("/api/categories/{id}/shares", {
      params: { path: { id: categoryId } },
      body: { email, permission: invitePermission },
    });
    setInviting(false);
    if (!response.ok || !data) {
      setInviteError(t("categories.sharing.inviteError"));
      return;
    }
    if (data.matched && data.share) {
      setShares((prev) => [...(prev ?? []), data.share as CategoryShare]);
      setInviteEmail("");
    } else {
  ...
  async function updatePermission(
    ...
    const { data } = await api.PATCH("/api/categories/{id}/shares/{userId}", {
      params: { path: { id: categoryId, userId } },
      body: { permission },
    });
    if (data) {
      setShares(
  ...
  async function performConfirmed() {
    ...
    const { response } = await api.DELETE(
      ...
      { params: { path: { id: categoryId, userId: share.user_id } } },
    );
  ...
  const canManage = category.permission === "owner";

  return (
    <section className="mx-auto flex w-full max-w-2xl flex-col gap-8 px-6 py-12 sm:px-10">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight">
          {t("categories.sharing.title", { category: category.name })}
        </h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          {t("categories.sharing.subtitle")}
        </p>
      </div>
        ...
        <li className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-black/10 px-4 py-3 dark:border-white/10">
          <div className="flex flex-col gap-0.5">
            <span className="font-medium">
              {category.owner_name || t("categories.sharing.unknownUser")}
            </span>
            <span className="text-xs text-zinc-500 dark:text-zinc-400">
              {t("categories.sharing.realOwner")}
            </span>
          </div>
        </li>
        ...
        {shares?.map((share) => (
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400">
                {t("categories.sharing.grantedBy", {
              ...
              {canManage ? (
                <select
                  ...
                  onChange={(e) =>
                    updatePermission(
                      share.user_id,
                      e.target.value as CategoryPermission,
                    )
                  }
                ...
                >
                  {PERMISSIONS.map((p) => (
                    <option key={p} value={p}>
                      {t(`categories.sharing.permission.${p}`)}
                    </option>
                  ))}
                </select>
              ) : (
                <span className="text-sm text-zinc-600 dark:text-zinc-400">
                  {t(`categories.sharing.permission.${share.permission}`)}
                </span>
              )}
      ...
      {canManage && (
        <section className="flex flex-col gap-3 border-t border-black/10 pt-6 dark:border-white/10">
          <h2 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400">
            {t("categories.sharing.inviteTitle")}
          </h2>
          <form
          ...
          >
            ...
            <label className="flex flex-col gap-1.5 text-sm font-medium">
              {t("categories.sharing.permissionLabel")}
              <select
                value={invitePermission}
                onChange={(e) =>
                  setInvitePermission(e.target.value as CategoryPermission)
                }
                className={inputClass}
              >
                {PERMISSIONS.map((p) => (
          ...
          {unmatched && (
            <div className="flex flex-col gap-2 rounded-md bg-black/[.04] px-4 py-3 text-sm dark:bg-white/[.06]">
              <p>
                {t("categories.sharing.unmatched", { email: unmatched.email })}
              </p>
              {unmatched.inviteAllowed ? (
                appInviteSent ? (
                  <p className="text-emerald-600 dark:text-emerald-400">
                    {t("categories.sharing.appInviteSent")}
                  </p>
                ) : (
                  <button
                    type="button"
                  ...
                  >
                    {t("categories.sharing.sendAppInvite")}
                  </button>
                )
              ) : (
                <p className="text-zinc-500 dark:text-zinc-400">
                  {t("categories.sharing.inviteUnavailable")}
                </p>
              )}
            </div>
          )}
            ...
            {confirming && (
              <>
                <DialogTitle className="text-base font-semibold">
                  {t(`categories.sharing.confirm.${confirming.kind}Title`)}
                </DialogTitle>
                <Description className="text-sm text-zinc-600 dark:text-zinc-400">
                  {t(`categories.sharing.confirm.${confirming.kind}Body`, {
