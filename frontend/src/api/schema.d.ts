...
export interface paths {
    ...
    "/api/version": {
        ...
        /**
         * The running build's release version and commit
         * @description Unauthenticated. Reports the git tag and commit the running backend was built from, stamped in at image-build time. Either field may be an empty string when it could not be determined (an untagged or unstamped build) — never null or omitted.
         */
...
export interface components {
    schemas: {
        ...
        BuildInfo: {
            /** @description The git commit hash the running build was made from, or an empty string when it could not be determined. Suffixed "-dirty" when built from a working copy with uncommitted changes. */
            commit: string;
            /** @description The git tag the running build was made from (e.g. "v0.4.2"), or an empty string when the build was not made from a tag. */
            version: string;
        };
        Category: {
            /** @description Optional opaque presentation token naming a client colour. Stored and echoed verbatim; never interpreted by the backend. Absent when unset. */
        ...
        Entry: {
            /** @description The currency of account_id, resolved server-side regardless of whether the caller currently holds any account-level access to that account — the same resolve-it-server-side precedent created_by_name establishes below, so amount can always be rendered correctly without a separate account fetch. This matters for an entry the caller can see only via a category permission with no account-level access at all (see category-sharing, entry-categories): account_currency is the one piece of that account's data carried onto the entry, every other detail (name, balance, other entries) stays unexposed. */
    ...
    getEntries: {
        parameters: {
            query?: {
                ...
                after?: string;
                /** @description Inclusive non-negative absolute amount lower bound in the fixed stored scale. */
                amount_from?: number;
                /** @description Inclusive non-negative absolute amount upper bound in the fixed stored scale. */
                amount_to?: number;
